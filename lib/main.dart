...
class _GameScreenState extends State<GameScreen> {
            ...
            child: SafeArea(
              child: Stack(
                children: [
                  ...
                  Positioned.fill(
                    child: AdaptiveGameArea(gameController: gameController),
                  ),
                  
                  // Botones de control flotantes (solo cuando no está jugando)
                  if (gameController.gameState.status != GameStatus.playing)
                    Positioned(
                      top: 10,
                      left: 10,
                      child: _buildFloatingControls(gameController),
                    ),
                  
                  // Información de estado flotante
                  if (gameController.gameState.status != GameStatus.playing)
                    Positioned(
                      bottom: 20,
                      left: 20,
                      right: 20,
                      child: Center(
                        child: _buildStatusMessage(gameController.gameState.status),
                      ),
                    ),
                ],
              ),
  ...
  Widget _buildFloatingControls(GameController gameController) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(
        color: GameColors.hudBackground.withValues(alpha: 0.95),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: GameColors.primary.withValues(alpha: 0.7),
          width: 2,
        ),
        boxShadow: [
          BoxShadow(
        ...
        children: [
          ...
          _buildControlButton(
            ...
            onPressed: () {
              ...
              // Forzar cambio de orientación del dispositivo
              SystemChrome.setPreferredOrientations([
          ...
          _buildControlButton(
            ...
            tooltip: gameController.gameState.isPlaying ? 'Pausar' : 'Jugar',
          ),
  ...
  Widget _buildStatusMessage(GameStatus status) {
    String message;
    Color color;
    IconData icon;
    ...
    switch (status) {
      case GameStatus.menu:
        message = 'Presiona PLAY para comenzar';
        color = GameColors.textSecondary;
      ...
      case GameStatus.gameOver:
        message = 'GAME OVER - Presiona REINICIAR';
  ...
  @override
  void dispose() {
    // Restaurar orientaciones automáticas al cerrar
    SystemChrome.setPreferredOrientations([
      DeviceOrientation.portraitUp,
      DeviceOrientation.landscapeLeft,
